...
| [`resilientclient`](./resilientclient) | `httpx` + `ratelimit` + `circuitbreaker`：为 outbound HTTP 提供 retry、节奏控制和 breaker 保护 |
| [`configlayers`](./configlayers) | `env` + `defaultmap`：加载基础配置，再按租户懒构建派生配置 |
